...
public class ItemSellerNPC extends IWorldNPC {
  ...
  private void handleWateringCanPurchase(Player player) {
    ...
    CustomItem wateringCan = plugin.getItemService().createItem("watering_can");
    ...
    if (!gamePlayer.getInventoryData().addItem(wateringCan)) {
  ...
  private void handleShovelPurchase(Player player) {
    ...
    CustomItem shovel = plugin.getItemService().createItem("shovel");
    ...
    if (!gamePlayer.getInventoryData().addItem(shovel)) {
      AlertService.sendError(player, "Инвентарь переполнен! Освободите место.");
      return;
    }
