...
import gg.cristalix.growagarden.model.garden.vegetation.SeedCategory;
...
public class CropBuyerNPC extends IWorldNPC {
  ...
  private void sellAllCrops(Player player) {
    ...
    for (Map.Entry<String, CropCustomItem> entry : crops.entrySet()) {
      ...
      SeedData seedData = seedService.getSeedDataByName(crop.getItemId());
      if (seedData == null) continue;

      List<CropCustomItem.CropInstance> instances = crop.getInstances();
      for (CropCustomItem.CropInstance instance : instances) {
      ...
      soldCrops.put(cropName, instances.size());

      gamePlayer.getInventoryData().removeItem(ItemEnum.CROP, entry.getKey());
    }
  ...
  private void showPrices(Player player) {
    ...
    for (SeedData seed : seeds.values()) {
      ...
      SeedCategory category = seed.getCategory();
      String categoryColor = category.getColorCode();
      ...
      lore.add("§7Категория: §f" + category.getDisplayName());
      lore.add("");
